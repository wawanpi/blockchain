        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-3xl border border-gray-100">
                <div class="p-8">
                    ...
                    {{-- 2. TABEL INVESTIGASI FORENSIK + TOMBOL RESTORE (FITUR BARU) --}}
                    @if($status === 'BAHAYA' && isset($perubahan) && count($perubahan) > 0)
                            ...
                            <div class="bg-red-600 px-6 py-3 border-b border-red-700 flex justify-between items-center">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="bg-red-800 text-white text-xs font-bold px-2 py-1 rounded border border-red-500">
                                        {{ count($perubahan) }} ITEM DIUBAH
                                    </span>
                                </div>

                                {{-- TOMBOL PULIHKAN DATA (RESTORE) --}}
                                <form action="{{ route('admin.transaksi.restore', $transaksi->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memulihkan data ini ke kondisi asli sesuai Snapshot Blockchain?');">
                                    @csrf
                    ...
                    {{-- 3. PERINGATAN JIKA SNAPSHOT JUGA DIMANIPULASI --}}
                    @if(isset($snapshotValid) && !$snapshotValid)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                            ...
                            <div class="text-xl font-bold text-gray-800 mt-1">TRX-{{ $transaksi->id }}</div>
                        </div>
                        <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                    ...
                    <div class="space-y-4 mb-8">
                        ...
                        <div class="relative group">
                            ...
                            <div class="p-4 bg-gray-100 rounded-xl font-mono text-xs break-all text-gray-600 border {{ $status !== 'AMAN' ? 'border-red-300 bg-red-50' : 'border-gray-200' }}">
                                {{ $recalculatedHash }}
                            </div>
                        </div>

                        {{-- Hash Asli Blockchain --}}
                        <div class="relative group">
                            <div class="flex justify-between items-end mb-1">
                            ...
                            <div class="p-4 bg-blue-50 rounded-xl font-mono text-xs break-all text-blue-800 border border-blue-200">
                                {{ $transaksi->data_hash ?? 'BELUM TERCATAT KE BLOCKCHAIN' }}
                            </div>
                        </div>
                    </div>

                    {{-- 6. BUKTI BLOCKCHAIN (EXTERNAL LINK) --}}
                    <div class="bg-gray-900 rounded-2xl p-6 text-white relative overflow-hidden">
                        ...
                        @if($transaksi->tx_hash_blockchain)
                            <div class="mb-4">
                                ...
                                <div class="font-mono text-sm text-gray-300 break-all bg-gray-800 p-3 rounded-lg border border-gray-700">
                                    {{ $transaksi->tx_hash_blockchain }}
                                </div>
                            </div>

                            <a href="https://amoy.polygonscan.com/tx/{{ $transaksi->tx_hash_blockchain }}" 
                               target="_blank"
                    ...
                    <div class="mt-8 text-center">
                        <a href="{{ route('admin.transaksi.index') }}" class="text-gray-500 font-medium hover:text-gray-800 transition-colors inline-flex items-center gap-2">
